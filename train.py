...
def parse_args():
    ...
    parser.add_argument('--load-from', help='the checkpoint file to load weights from')  # 要从中加载权重的检查点文件
    parser.add_argument('--resume-from', help='the checkpoint file to resume from')  # 要从中恢复的检查点文件
    group_gpus = parser.add_mutually_exclusive_group()
    group_gpus.add_argument('--gpu_ids', type=int, default='0', help='id of gpu to use ''(only applicable to non-distributed training)')
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
